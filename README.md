...
**The various phenomena connected with the theory are all aspects of a single thing: the structure of space-time.**
(We can take the Lorentz Transformations as the definition of that structure.)
A beautiful example of this is the connection between aberration and the Doppler effect (optical effects) on the one hand, and the flattening effect (a geometrical effect) on the other hand.
To see the connection, look at the boost of a Doppler cone. 
(Here, a Doppler cone is a generated by a wave vector k<sup>i</sup> having a given frequency, but different spatial directions. It's not the same thing as a light cone.)
In an unboosted grid, the top of the Doppler cone forms a circle.
In a boosted grid, the circle morphs into an ellipse.
The exact same morphing applies to events instead of wave vectors.
You can view the circle-ellipse shapes as a time-slices across a history.
(The Lorentz Transformations are *agnostic*, in the sense that they apply to different kinds of physical quantities.)
If you take the ellipse to represent events instead of wave vectors, then a corresponding boost transforms the ellipse into a circle.
