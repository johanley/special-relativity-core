...
(That structure can be taken as defined by the Lorentz Transformations.)
A beautiful example of this is the connection between aberration and the Doppler effect (optics) on the one hand, and the flattening effect (geometry) on the other hand.
To see the connection, look at the boost of a Doppler cone. 
(Here, a Doppler cone is a generated by a wave vector k<sup>i</sup> having a given frequency, but different spatial directions. It's not the same as a light cone.)
In an unboosted grid, the top of the Doppler cone forms a circle.
In a boosted grid, the circle morphs into an ellipse.
The exact same geometry applies to events instead of wave vectors.
