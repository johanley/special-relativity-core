...
public final class CompareFormulaWithLT extends TextOutput {
  ...
  void explore() {
    lines.add("Compare results from the velocity transformation formula with results directly from the Lorentz transformation."+Util.NL);
    lines.add("With no loss of generality, in K we have:");
    lines.add(" - a object velocity v in K in any old direction");
  ...
  private void formula(double boost_speed, Velocity v) {
    lines.add("Using the velocity transformation formula we get:");
    Velocity boost = Velocity.of(X, boost_speed);
    lines.add("Boost: " + boost + " Velocity v_K:" + v);
