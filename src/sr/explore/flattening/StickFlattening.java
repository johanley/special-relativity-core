...
public class StickFlattening extends TextOutput {
  ...
  */
  void stickAlongAxis(List<String> lines, Double β) {
    ...
    History2 histA = UniformVelocity.stationary(Position.of(X, 1.0));
    History2 histB = UniformVelocity.stationary(Position.of(X, 2.0));
  ...
  */
  void stickAngledToXAxis(List<String> lines, Double β) {
    ...
    History2 histA = UniformVelocity.stationary(Position.origin()); 
    History2 histB = UniformVelocity.stationary(Position.of(1.0, 1.0, 0.0)); //other end of the stick
  ...
  */
  void stickAngledToXAxisWithEquivalentBoostParams(List<String> lines) {
    ...
    double L0 = 1.0;
    History2 histA = UniformVelocity.stationary(Position.origin()); 
    History2 histB = UniformVelocity.stationary(Position.of(L0*Math.cos(restAngle), L0*Math.sin(restAngle), 0.0)); //other end of the stick
    Event diff = histB.event(0.0).minus(histA.event(0.0));
    lines.add("K b-a:" + diff);
    lines.add("K stick length:" + diff.spatialMagnitude());
